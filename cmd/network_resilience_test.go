...
func TestNetworkResilience(t *testing.T) {
	testCases := []struct {
		...
		{"small_network_mild_drops", 20, 0.05, 0.1},    // Reduced from 100
		{"medium_network_heavy_drops", 50, 0.15, 0.05}, // Reduced from 500
		{"tiny_network_extreme_drops", 10, 0.30, 0.2},  // Reduced from 50
	}
...
func testNetworkResilience(t *testing.T, nodeCount int, dropRate, failureRate float64) {
	...
	for i := 0; i < nodeCount; i++ {
		addr := Address{IP: "127.0.0.1", Port: 9000 + i}
	...
	bootstrapCount := min(5, nodeCount) // Use fewer bootstrap nodes
	...
	dataCount := min(10, nodeCount/2) // Reduce data count for smaller networks
	...
	baseExpectedRate := 0.5 // Start with 50% as base expectation
	impactFactor := failureRate + dropRate
	expectedMinRate := maxFloat(0.2, baseExpectedRate*(1.0-impactFactor))
	...
	if successRate < expectedMinRate {
		t.Errorf("Success rate %.1f%% below expected minimum %.1f%% (with %.1f%% failures and %.1f%% drops)",
